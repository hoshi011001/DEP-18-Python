...
def ingest_data(**kwargs):
    ...
    load_data_adls(df, kwargs['folderName'], kwargs['fileName'])
...
def transform_retail(**kwargs):
    ...
    load_data_adls(df_enunciado, 'gold', kwargs['enunciado'])
...
def load_data_mysql(**kwargs):
    engine = db.create_engine("mysql://root:root@192.168.100.77:3310/retail_db")
    conn = engine.connect()
    df = read_data_adls('datalake','gold', kwargs['enunciado'])
    df.to_sql(kwargs['enunciado'], engine, index=False, if_exists='replace')


def enunciado1():
        df_customer = read_data_adls('datalake','landing', 'customers')
        df_orders = read_data_adls('datalake','landing', 'orders')
        df_items = read_data_adls('datalake','landing', 'order_items')
...
def enunciado2():
    ...
    df_products = read_data_adls('datalake','landing', 'products')
    df_categories = read_data_adls('datalake','landing', 'categories')
    df_items = read_data_adls('datalake','landing', 'order_items')
...
with dag:
    ...
    [run_ingest_customer, run_ingest_orders, run_ingest_order_items] >> transform >> [run_load_enunciado1, run_load_enunciado2]